...
SensorContainer::SensorContainer(QWidget *parent)
 : QWidget(parent),
   ogRenderer(this), 
   mapControlPanel(this)
{
    QVBoxLayout *vLayout = new QVBoxLayout; 
    vLayout->addWidget(&ogRenderer,4); 
    vLayout->addWidget(&mapControlPanel,1); 
    setLayout(vLayout); 
    connect(&mapControlPanel.showGrids, SIGNAL(stateChanged(int)),&ogRenderer,SLOT(setShowGrids( int ))); 
    connect(&mapControlPanel.showOGs, SIGNAL(stateChanged(int)),&ogRenderer,SLOT(setShowOGs( int ))); 
    connect(&mapControlPanel.showRobots, SIGNAL(stateChanged(int)),&ogRenderer,SLOT(setShowRobots( int ))); 
    connect(&mapControlPanel.showLabels, SIGNAL(stateChanged(int)),&ogRenderer,SLOT(setShowSnaps( int ))); 
    connect(&mapControlPanel.showPointclouds, SIGNAL(stateChanged(int)),&ogRenderer,SLOT(setShowPointclouds( int )));
    connect(&mapControlPanel.showPatchBorders, SIGNAL(stateChanged(int)),&ogRenderer,SLOT(setShowPatchBorders( int )));
    connect(&mapControlPanel, SIGNAL(propsChanged()), &ogRenderer, SLOT(update()));  
    connect(&ogRenderer, SIGNAL(moveMOLeft()), &(mapControlPanel.xSB), SLOT(stepDown())); 
    connect(&ogRenderer, SIGNAL(moveMORight()), &(mapControlPanel.xSB), SLOT(stepUp())); 
    connect(&ogRenderer, SIGNAL(moveMOUp()), &(mapControlPanel.ySB), SLOT(stepUp())); 
    connect(&ogRenderer, SIGNAL(moveMODown()), &(mapControlPanel.ySB), SLOT(stepDown())); 
    connect(&ogRenderer, SIGNAL(yawMOPos()), &(mapControlPanel.yaSB), SLOT(stepUp())); 
    connect(&ogRenderer, SIGNAL(yawMONeg()), &(mapControlPanel.yaSB), SLOT(stepDown()));
}

//void MapEdit::setMapManager( QTMapDataInterface *mapManager){
//    ogRenderer.setMapManager(mapManager); 
//    mapControlPanel.setMapManager(mapManager);
//}
...
void MapControlPanel::handleSelection(){
...
void MapControlPanel::setToRoot(){
    xSB.setValue(0);
    ySB.setValue(0); 
    zSB.setValue(0); 
    rSB.setValue(0); 
...
void MapControlPanel::updateSelectedObject(double){
...
void MapControlPanel::updateSelectedObject(int value){
    updateSelectedObject((double) value); 
}

//void MapControlPanel::setActionValues(MapObject *mo){
...
//void MapControlPanel::setMapManager( QTMapDataInterface *in_mapManager){
...
void MapControlPanel::captureMap(){
    ...
    sleep(1);
    if(ok){
	QImage capturedMap = ((SensorContainer *) parent())->ogRenderer.captureMap();
	//mapManager->addMapSnapshot(capturedMap, comment); 
    }
}

void MapControlPanel::exportHtml(){
...
void MapControlPanel::updateMap(){
