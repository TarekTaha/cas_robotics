...
global scan Q r PointData IntensityData RangeData robot_maxreach vid_object;
...
%     hCOM.Laser.registerevent(@myhandler)
display('deleted gavins myhandler for listening to scasnner returns');
...
%         '. Scanning through: ', num2str(tilt_scan_range)));
    ...
    pause(0.05);
    hCOM.Laser.Start(tilt_scan_range);
    
    pause(0.5);
    
    %if not started their must be a problem or if no data has started to be returned
    if(hCOM.Laser.Started==0 && get(hCOM.Laser,'Completed')==0)   
        release_scanner(hCOM.Laser);
    ...
    elseif isempty(PointData)
        ...
        while get(hCOM.Laser,'Completed')==0
            pause(0.5);display('.'); 
...
while get(hCOM.Laser,'Completed')==0 || isempty(PointData)
    %display('waiting in scan');
    pause(0.5);
     if temp_counter<=0
         response=input(strcat('Completed=',num2str(get(hCOM.Laser,'Completed')),...
