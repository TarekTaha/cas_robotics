...
%
% _targetdist_ (structure) Distance between desired & actual target
%
% _dist_val_ (double) Distance between desired & actual pt in space

function [qt,solutionvalid,used_sol,targetdist,dist_val] = sandblasting_pose_sel(robot, pt, targetNormal, q, quickver)
...
%% Check the optimisation results, change start to all Zeros if necessary
    [valid,dist(1),targetdist(1).val]=check_newQ(qt,qlimits,pt,t,Links,numlinks,desired_targ); 
    if valid; used_sol=1; dist_val= dist(1); qt=qt'; return;
        ...
        if ~isempty(find(q(1:3)~=0, 1))
            ...
            [valid,dist(2),targetdist(2).val]=check_newQ(qt,qlimits,pt,t,Links,numlinks,desired_targ); 
...
%% Extended find (2 extra starting qs)    
        ...
        [valid,dist(3),targetdist(3).val]=check_newQ(qt,qlimits,pt,t,Links,numlinks,desired_targ);
        ...
        [valid,dist(4),targetdist(4).val]=check_newQ(qt,qlimits,pt,t,Links,numlinks,desired_targ);
...
function [valid,dist,targetdist]=check_newQ(qt,qlimits,pt,t,Links,numlinks,desired_targ)
    global optimise densoobj workspace;
    ...
    targetdist=dist2targetpoint(tr,desired_targ);
...
function [targetdist]=dist2targetpoint(tr,desired_targ)
