function result=AT2_bridgeSection
cartYoffset=0.11;
...
vertices(numverts+1,:)=(vertices(41,:)+vertices(42,:))/2+[-0.7,0,0];       
...
vertices(numverts+5,:)=(vertices(12,:)+vertices(11,:))/2 +[-0.7,0,0];       
...
%Add extra vertices for the crossbeam in the FRONT         
        numverts=size(vertices,1);
        %RIGHT crosbeam vert
        %41 is back right wall bottom end %42 is front right wall bottom end
        %so it is 7cm away from mid towards back
% test 6 
vertices(numverts+1,:)=(vertices(41,:)+vertices(42,:))/2+[0.9,0,0];       
%test10
%         vertices(numverts+1,:)=(vertices(41,:)+vertices(42,:))/2+[-0.2,0,0];       
        ...
        %so it is 7cm away from mid towards back
% test6  
vertices(numverts+5,:)=(vertices(12,:)+vertices(11,:))/2 +[0.9,0,0];       
%test10
%       vertices(numverts+5,:)=(vertices(12,:)+vertices(11,:))/2 +[-0.2,0,0];       
        ...
        plywrite(data,'AT2_bridgeSection.ply','ascii');
    end
