        ...
        Classifier()
{
    ...
    strcpy(friendlyName, "SURF Classifier");
    classifierType = SURF_FILTER;

    // append identifier to directory name
    strcat(directoryName, FILE_SURF_SUFFIX);
}

SurfClassifier::SurfClassifier(const char * pathname) :
        Classifier(pathname)
{
    // set the type
    classifierType = SURF_FILTER;
...
void SurfClassifier::load(const char * fileName)
{
...
void SurfClassifier::load(string fileName)
{
    load(fileName.c_str());
}
bool SurfClassifier::containsSufficientSamples(TrainingSet *sampleSet)
{
...
ClassifierOutputData SurfClassifier::classifyFrame(IplImage *frame)
{
    cvZero(guessMask);
    if (!isTrained)
        return outputData;
...
void SurfClassifier::updateSiftImage()
{
    IplImage *featureImage = cvCloneImage(sampleCopy);
