...
void RedrawMap(GtkWidget * widget) // Redraws the Map from the saved and buffered file, Was used for testing only
	...
	if (pixbuf)
  	...
  	if(spacefile!=NULL)
  		*spacefile='\0';   
  	spacefile=g_strdup_printf("%s%s",file,"_FreeSpace.png");
...
//			}
  	...
  	if(savefile!=NULL)
  		*savefile='\0';   
  	savefile=g_strdup_printf("%s%s",file,"_PATH.png");
  	gdk_pixbuf_save(pixbuf,savefile,"png",NULL,NULL);//save the file
  	g_free(savefile);
  	g_free(file);
