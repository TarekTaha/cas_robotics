function obsProbs = obsFactored(tasksFile)
...
for i=1:numSpatialStates
   for j=1:length(obsStrings)
        ...
        ss = sum(destProbs(indx,1:6));
        if(j==1)
           if ss ~= 0
                fprintf(fileid,'\t\t(s%d	(intention	(%s\t\t(destination (d1 (%f)) (d2(%f)) (d3 (%f)) (d4 (%f)) (d5 (%f)) (d6 (%f)) ))\n',i,obsStrings{j},destProbs(indx,1),destProbs(indx,2),destProbs(indx,3),destProbs(indx,4),destProbs(indx,5),destProbs(indx,6));			                
           else
                fprintf(fileid,'\t\t(s%d	(intention	(%s\t\t(destination (d1 (%f)) (d2(%f)) (d3 (%f)) (d4 (%f)) (d5 (%f)) (d6 (%f)) ))\n',i,obsStrings{j},1/6,1/6,1/6,1/6,1/6,1/6);			               
           end
        else
            if ss ~=0
                ...
                fprintf(fileid,'\t\t\t\t\t(%s\t\t(destination (d1 (%f)) (d2(%f)) (d3 (%f)) (d4 (%f)) (d5 (%f)) (d6 (%f)) ))\n',obsStrings{j},1/6,1/6,1/6,1/6,1/6,1/6);			                        
            end
        end
   end
   fprintf(fileid,'\t\t\t)\n\t\t)\n');   
